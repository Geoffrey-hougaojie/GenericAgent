...
### 步骤4.5：匹配任务分解模板（来自经验库 patterns.md）

> **PEEU核心发现**：高层任务分解（先做什么后做什么的顺序）比低层工具选择更值钱。
> 写plan前先对号入座，套用下面的模板确定步骤顺序，再填SOP。别每次从零开始想。

读 `D:\知识库\GA自学\outputs\experience_reuse\patterns.md`，按任务类型匹配模板：

| 任务类型 | 触发词 | 高层分解（步骤顺序） | 最大坑（写plan时必须避免） |
|---------|--------|---------------------|--------------------------|
| 研究学习 | 学习/研究/arxiv/reading | 交叉检查→读源→搜索→费曼→改进→记忆 | 跳过交叉检查导致重复研究 |
| 信息聚合 | 晨报/digest/聚合/brief | 多源采集→去噪→结构化→推送→入库 | 单点故障无fallback |
| Git同步 | sync/upstream/push/merge | fetch→分类→合并→重打补丁→提交 | 本地补丁被覆盖没发现 |
| 健康监控 | health/reflection/检查 | 扫描→统计→对比→深度诊断→建议 | 信旧报告不信源码 |
| 质量优化 | optimize/优化/质量 | 读队列→扫描→分类→人工复查→候选 | 自动分类不可靠 |
| Web操作 | web/浏览器/页面/API | 导航→等渲染→提取→验证→fallback | 首扫定论/没等渲染 |
| 守护进程 | daemon/scheduler/进程 | 存活检查→资源泄漏→日志健康→诊断→修复 | 端口残留阻塞重启 |

**匹配规则**：
1. 任务名或需求描述含触发词 → 选对应类型
2. 匹配到多个 → 选最具体的（如 morning_brief 既是信息聚合又含Web操作→选信息聚合为主，Web操作为子步骤）
3. 无匹配 → 自建分解，但参考「跨模式通用原则」（patterns.md末尾6条原则）
4. **匹配后必须读 patterns.md 对应模式的「常见坑」表**，把相关坑写入plan的「风险/不确定点」
